...
const settingsHelpZhCN: SettingsHelpMap = {
  ...
  'settings.data_source.TUSHARE_API_MODE': {
    title: 'Tushare 接入模式',
    summary: '选择官方 Tushare Pro API 或 Relay 网关。',
    usage: '填 tushare 走官方 API；填 relay 时需要同时配置 Base URL 和 Relay Key。',
    valueNotes: ['默认 tushare，历史部署无需改动。', 'relay 会把接口名追加到 Relay Base URL 的路径中。'],
    impact: ['决定 Tushare 数据源使用官方 POST 结构还是网关 GET 结构。'],
    notes: ['Relay 网关错误会继续触发既有数据源 fallback。'],
  },
  'settings.data_source.TUSHARE_RELAY_BASE_URL': {
    title: 'Tushare Relay Base URL',
    summary: 'Relay 网关服务根地址。',
    usage: '填入以 http:// 或 https:// 开头的完整根地址，例如 https://relay.example.com/tushare/pro。',
    valueNotes: ['仅在 TUSHARE_API_MODE=relay 时生效。'],
    impact: ['决定 Relay 请求的 URL 根和接口路径拼接方式。'],
    notes: ['不要把接口名写进 Base URL；客户端会在其后拼 /daily 等接口名。'],
  },
  'settings.data_source.TUSHARE_RELAY_KEY': {
    title: 'Tushare Relay Key',
    summary: 'Relay 网关客户端 Key。',
    usage: '填入网关提供的客户 Key，客户端会放在 X-API-Key Header 中。',
    valueNotes: ['TUSHARE_API_MODE=relay 时必填。'],
    impact: ['影响 Relay 网关鉴权和可用接口。'],
    notes: ['不要把真实 Key 提交到仓库或公开日志。'],
  },
...
const settingsHelpEnUS: SettingsHelpMap = {
  ...
  'settings.data_source.TUSHARE_API_MODE': {
    title: 'Tushare Access Mode',
    summary: 'Select the official Tushare Pro API or a Relay gateway.',
    usage: 'Use tushare for the official API. Use relay with a Relay Base URL and client key.',
    valueNotes: ['The value defaults to tushare.', 'Relay appends the API name to the Relay Base URL path.'],
    impact: ['Controls whether Tushare uses the official POST contract or the Relay GET contract.'],
    notes: ['Relay errors continue through the existing data-source fallback.'],
  },
  'settings.data_source.TUSHARE_RELAY_BASE_URL': {
    title: 'Tushare Relay Base URL',
    summary: 'Service root for the Tushare Relay gateway.',
    usage: 'Use a complete http:// or https:// URL, for example https://relay.example.com/tushare/pro.',
    valueNotes: ['Effective only when TUSHARE_API_MODE=relay.'],
    impact: ['Determines the URL root and path joining for Relay requests.'],
    notes: ['Do not append API names to the Base URL; the client adds them.'],
  },
  'settings.data_source.TUSHARE_RELAY_KEY': {
    title: 'Tushare Relay Key',
    summary: 'Client key for the Tushare Relay gateway.',
    usage: 'Paste the gateway client key. The client sends it in the X-API-Key header.',
    valueNotes: ['Required when TUSHARE_API_MODE=relay.'],
    impact: ['Controls Relay gateway authentication and available APIs.'],
    notes: ['Do not commit the real key or print it in public logs.'],
  },
  'settings.data_source.TICKFLOW_API_KEY': {
    title: 'TickFlow API Key',
